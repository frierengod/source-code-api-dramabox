<?php
/**
 * Contoh Script Bridge API Dramabox via StreamAPI.web.id
 * ----------------------------------------------------
...
// Fungsi bantuan buat manggil endpoint Dramabox di sistem Bridge
function callDramaboxAPI($urlTujuan, $token) {
...
// UJI COBA KODE
// -----------------------------------------------------------------
$baseUrl = "https://streamapi.web.id/p/dramabox/api/v1";

// Contoh 1: Fitur Cari Film Dramabox
echo "\n=====================[ 1. Pencarian Dramabox ]==================\n";
$kataKunci = urlencode("cinta");
$halaman = 1;
$urlCari = "{$baseUrl}/search/{$kataKunci}/{$halaman}?lang=in";

$hasilCari = callDramaboxAPI($urlCari, $API_TOKEN);
if ($hasilCari) {
...
$hasilDetail = callDramaboxAPI($urlDetail, $API_TOKEN);
if ($hasilDetail) {
...
$hasilNonton = callDramaboxAPI($urlNonton, $API_TOKEN);
if ($hasilNonton) {
...
echo "▶▶▶ https://streamapi.web.id/docs/dramabox ◀◀◀\n\n";
